...
## Thesis Prototype Description (v10.1)

As a practical contribution, we provide a system architecture and a fully functional prototype built on Hyperledger Fabric 2.5. The smart contract layer is written in Go using the `fabric-contract-api-go` SDK and comprises 10 contract namespaces exposing approximately 85 functions that cover the complete Guarantee of Origin lifecycle — issuance, transfer, cancellation, cross-channel conversion, bridge transfers, metering backlog accumulation, device registration, administration, querying, and oracle data integration — across four energy carriers (electricity, hydrogen, biogas, and heating/cooling). The network is organised into two permissioned channels (electricity-de and hydrogen-de) aligned with carrier-level market separation, each hosting its own issuer, producer, and buyer organisations governed by a shared four-node Raft orderer cluster. An off-chain application layer, implemented in TypeScript with Node.js and the Fabric Gateway SDK, exposes a REST API consumed by a React front-end. Performance is evaluated using Hyperledger Caliper, with JavaScript workload modules covering both read-heavy query paths and write-heavy issuance and conversion flows. The prototype was developed over ten iterative design cycles, with each cycle producing quantitative benchmark evidence used to inform architectural decisions, demonstrating that permissioned distributed ledger technology can sustain the throughput requirements of national GO markets while preserving participant confidentiality through private data collections and hash commitments.
