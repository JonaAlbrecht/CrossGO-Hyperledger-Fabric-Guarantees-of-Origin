#!/bin/bash
# Manual deployment of golifecycle v10.1 to both channels
# Don't exit on errors (chaincode may already be installed)

REPO_DIR="/root/hlf-go/repo"
PKG_FILE="$REPO_DIR/golifecycle.tar.gz"
...
peer lifecycle chaincode install "$PKG_FILE" 2>&1 | grep -E 'Installed|already' || true
echo "✓ eissuer"
